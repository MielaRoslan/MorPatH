...
class NCC_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        data = {}
        data_label = {}
        target_y = np.array(self.target[index], dtype=np.float32)
        data_label['label'] = torch.from_numpy(target_y).type(torch.FloatTensor)
